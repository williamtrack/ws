...
bool VideoCapture::Load(const string& url) {
    mUrl = url;
...
bool VideoCapture::LoadCam(const std::string& url)
{
    mUrl = url;

    avdevice_register_all();
    AVInputFormat* av_input_format = const_cast<AVInputFormat*>(av_find_input_format("dshow"));
    avformat_open_input(&mFormatContext, mUrl.c_str(), av_input_format, nullptr);

    return init();
}

AVPixelFormat VideoCapture::GetPixelFormat() {
    return mPixelFormat;
}
...
bool VideoCapture::init()
{
    ...
    if (avformat_find_stream_info(mFormatContext, NULL) < 0) {
        cout << "can't find stream info in " << mUrl << endl;
        return false;
    }
    ...
    if (mVideoStreamIndex < 0) {
        cout << "can't find video stream in " << mUrl << endl;
        return false;
    }
    std::cout << "stream index = " << mVideoStreamIndex << std::endl;

    // 获取视频轨道的解码器相关参数
    AVCodecParameters* codecParam = mFormatContext->streams[mVideoStreamIndex]->codecpar;
    cout << "codec id = " << codecParam->codec_id << endl; //AVCodecID 7 AV_CODEC_ID_MJPEG
    cout << "format id = " << codecParam->format << endl; //AVPixelFormat 13 AV_PIX_FMT_YUVJ422P
...
AVFrame* VideoCapture::NextFrame() {
...
void VideoCapture::DumpVideoInfo() {
    av_dump_format(mFormatContext, 0, mUrl.c_str(), 0);
}

int VideoCapture::GetVideoWidth() {
    return mVideoWidth;
}

int VideoCapture::GetVideoHeight() {
    return mVideoHegiht;
}
