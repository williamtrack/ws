...
bool camera_capture::Load(const std::string& url, int width, int height)
{
    this->width = width;
    this->height = height;
    ...
    packet = av_packet_alloc(); //对指针类型一定要先初始化，如果packet非指针则无需初始化
    if (!newpkt) {
...
AVFrame* camera_capture::NextFrame()
{
    // read data from device
    while ((ret = av_read_frame(fmt_ctx, packet) == 0) && (count++ < 100)) {
    //if (ret = av_read_frame(fmt_ctx, packet) == 0) {
        int i = 0;
        av_log(NULL, AV_LOG_INFO, "packet size is %d(%p)", packet->size, packet->data);
        // YUYVYUYVYUYVYUYV   YUYV422
        // YYYYYYYYUUVV YUV420
        yuyv422ToYuv420p(frame, packet, width, height);
        ...
        //
        av_packet_unref(packet); // release pkt
    }
    //else {
    //    return NULL;
    //}
